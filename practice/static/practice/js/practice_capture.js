/*
 * practice_capture.js
 * Continuous practice loop: watches the camera, detects a sign, and on a
 * confident/steady match POSTs to /practice/answer/, which scores it
 * (+10 / -5) and returns the next random letter + reference image.
 * Same MediaPipe setup, normalization math, and charge/commit debounce
 * as hand_capture.js (used by Tests) — just no "finish" state, since
 * Practice runs indefinitely.
 */
(function () {
  const cfg = window.SIGNLEARN_PRACTICE;
  if (!cfg) return;

  const NUM_LANDMARKS = 21;
  const FEATURES_PER_HAND = NUM_LANDMARKS * 3;
  const TOTAL_FEATURES = FEATURES_PER_HAND * 2;

  const CHARGE_BUFFER_SIZE = 10;
  ...
  let currentTarget = cfg.targetLetter;
  let chargeBuffer = [];
  ...
  async function submitAnswer(letter, confidence) {
    ...
    const resp = await fetch(cfg.answerUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-CSRFToken": cfg.csrfToken },
      body: JSON.stringify({ letter: letter, confidence: confidence, target_letter: currentTarget }),
    });
    ...
    pointsText.textContent = data.total_points;

    feedbackText.textContent = data.is_correct
      ? `Correct! +${data.points_awarded} points`
      : `Not quite \u2014 ${data.points_awarded} points`;
    feedbackText.className = data.is_correct ? "feedback-correct" : "feedback-incorrect";

    currentTarget = data.next_letter;
    targetLetterEl.textContent = currentTarget;
    if (referenceImage && data.next_image_url) {
      referenceImage.src = data.next_image_url;
    }
    predictionText.textContent = "";
    chargeBuffer = [];
    committedLabel = null;

    statusText.textContent = "Show the sign for " + currentTarget;
